...
class ProductImageZoom {
  ...
  init() {
    ...
    this.previewHeader.innerHTML = `
      ...
      <span>ZOOM PREVIEW &bull; ${this.zoomLevel}X</span>
    `;
  ...
  _positionPreviewPanel() {
    ...
    // Match the height of the image container, up to 520px max width
    const targetH = Math.min(Math.max(340, Math.round(cRect.height)), viewportH - 24);
    let targetW = Math.min(520, Math.round(cRect.width * 1.05));

    // Check space on right side
    const spaceRight = viewportW - cRect.right - 20;
    const spaceLeft = cRect.left - 20;
    ...
    this.actualLensW = Math.max(40, Math.min(cRect.width * 0.85, Math.round(targetW / this.zoomLevel)));
    this.actualLensH = Math.max(40, Math.min(cRect.height * 0.85, Math.round(targetH / this.zoomLevel)));
